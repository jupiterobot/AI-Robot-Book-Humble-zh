...
def exec_load(loader):
    # 参考了 Command 的测试中 Command substitution 的执行方式
...
def test_port_name():
    # 修改 port_name，期望该端口名称被设置到 xacro 中
    rdl = RobotDescriptionLoader()
...
def test_use_gazebo():
    # 修改 use_gazebo，期望 xacro 中包含 gazebo_ros2_control
    rdl = RobotDescriptionLoader()
    rdl.use_gazebo = 'true'
...
def test_use_ignition():
    # 同时修改 use_gazebo 和 use_ignition，期望 xacro 中包含 ign_ros2_control
    rdl = RobotDescriptionLoader()
    rdl.use_gazebo = 'true'
...
def test_use_camera():
    # 修改 use_camera，期望 xacro 中包含 camera
    rdl = RobotDescriptionLoader()
    rdl.use_camera = 'true'
    rdl.gz_control_config_package = 'crane_plus_description'
    rdl.gz_control_config_file_path = 'test/dummy_controllers.yaml'
