...
class Xf_StringCommandActionClient:
...
class xf_SpeechClient(Node):
    def __init__(self):
        super().__init__('xf_speech_client')
        self.get_logger().info('正在启动语音对话节点...')
        self.recognition_client = Xf_StringCommandActionClient(
            self, 'xf_recognition/command')
        self.synthesis_client = Xf_StringCommandActionClient(
            self, 'xf_synthesis/command')
