...
class NavigationActionServer(Node):
    def __init__(self):
        ...
        self._nav_action_server = ActionServer( # 创建 ActionServer
            self,                               # 指定 ROS 节点
            StringCommand,                      # 指定 Action 类型
            'ps_navigation/command',            # 指定 Action 名称
            self.execute_callback)              # 将接收到的 goal 交由 callback 函数处理

    def execute_callback(self, goal_handle):
        self.get_logger().info(f'开始向目标坐标 `{goal_handle.request.command}` 移动')
        ...
        for i in range(wait_time):
            ...
            if 0.95 > prob:
                ...
                self.get_logger().info(f'正在向目标坐标 `{goal_handle.request.command}` 移动')

                feedback_msg.process = 'navigating' if i < wait_time-1 else 'reached' # 中间反馈设为 `navigating`，最终设为 `reached`
                goal_handle.publish_feedback(feedback_msg) # 将中间结果作为反馈发布

            else:
                # [失败] 当 prob 值大于等于 0.95 时执行（概率 5%）
                self.get_logger().info(f'向目标坐标 `{goal_handle.request.command}` 的移动失败了')
        ...
        self.get_logger().info(f'成功到达目标坐标 `{goal_handle.request.command}`')
...
def main(args=None):
    ...
    navigation_action_server = NavigationActionServer() # 声明 NavigationActionServer() 类

    rclpy.spin(navigation_action_server) # 启动 ActionServer 的回调函数
