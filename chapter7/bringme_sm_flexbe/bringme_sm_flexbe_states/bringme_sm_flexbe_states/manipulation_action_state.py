...
class ManipulationActionState(EventState):
    """
    通过 Action 通信启动物体操作，并将结果存入 userdata.text
    ...
    Userdata
    ># target     string   要操作的目标物体名称（string 类型）（输入）
    #> text       string   Action 执行后的结果（string 类型）（输出）
    ...
    def execute(self, userdata):
        ...
        if self._client.has_result(self._topic):
            ...
            if userdata.text == 'failed':
                Logger.logwarn('物体操作失败')
                self._return = 'failed'
                return self._return  # 返回 'failed'
            else:
                Logger.loginfo(f'物体操作结果: {userdata.text}')
                self._return = 'done'
    ...
    def on_enter(self, userdata):
        ...
        if 'target' not in userdata:
            self._error = True
            Logger.logwarn("执行 ManipulationActionState 需要 userdata.target！")
            return
