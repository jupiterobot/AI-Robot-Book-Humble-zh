...
class NavigationActionState(EventState):
    """
    通过 Action 通信启动前往目的地的导航，并将结果存入 userdata.text
    ...
    参数
    -- timeout             最大允许时间（秒）
    -- action_topic        导航的 Action 名称

    输出
    <= done                导航成功时
    <= failed              因任何原因失败时
    <= canceled            用户请求取消时
    <= timeout             超过前往目的地的最大允许时间时

    Userdata
    ># destination  string 目的地名称（string 类型）（输入）
    #> text         string Action 执行导航的结果（string 类型）（输出）
    ...
    def execute(self, userdata):
        ...
        if self._client.has_result(self._topic):
            ...
            if userdata.text == 'failed':
                ...
                Logger.loginfo(f'导航结果: {userdata.text}')
                self._return = 'done'
    ...
    def on_enter(self, userdata):
        ...
        # 检查 userdata 中是否存在 'destination'
        if 'destination' not in userdata:
            self._error = True
            Logger.logwarn("执行 NavigationActionState 需要 userdata.destination！")
            return

        # 检查输入值是否为 string 类型
        if not isinstance(userdata.destination, (str)):
            self._error = True
            Logger.logwarn('输入类型为 %s，但需要 string 类型', type(userdata.destination).__name__)
